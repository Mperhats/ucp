...
add tx_gas_used, tx_effective_gas_price as data options. requires eth_getBlockReceipts
...
bugfix: reorg may halt progress until restart. more details
v1.4
2f76 2024-04-29

index trace_block data see ’trace_*’ in block data fields
filter numerical data. new filter ops: eq, ne, lt, gt. see filter operations
...
Linux or Mac
URL to an Ethereum node (Alchemy, Geth)
...
To install Shovel, you can build from source (see build from source) or you can download the binaries
...
batch_size and concurrency defined in eth_sources Config
Data Selection
...
Must be one of: bool, byte, bytea, smallint, int, bigint, numeric, text.
...
Indexed dynamic ABI types (string, bytes, arrays, tuples) are stored as the 32-byte topic hash, not the original value. Store them in a bytea column and name it with a `_hash` or `_topic` suffix to avoid confusion.
